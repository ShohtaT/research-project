...
def generate_prompt(data: str) -> str:
    ...
    prompt = (
        "一旦これまでにリクエストした内容の記憶は全てリセットしてください。"
        "これは、あるドラマ作品の中で特定のBGMがかかっている間に行われた会話のテキストについてアノテーションしたデータです。"
        "「BGM名」と「会話テキスト」を含んで作成したアノテーションデータ（A）です。"
        "これにより、どんな会話にどんなBGMが使用されているのかという特徴が見えてくるはずです。"
        f"\n {dataset}\n"
        "さて、これを用いて、以下のことを行なってください。"
        "以下のデータは、実際に行われた会話です。これに類似する会話テキストを探索し、Aに対応するBGM名を、類似度1位から順に上位5つ提示してください。"
        "会話の文脈が大きく異なっている可能性がありますが、それでも使用されている単語やテーマから、類似度をランクづけしてください。"
        "会話の類似度は、会話テキストの文脈やテーマ、使用されている単語がどれだけ似ているかで判断してください。"
        "時間はかけても構いません。ずべてのアノテーションデータ（A）に対してきちんと類似度を比較し、探索を行ってください。"
        "なお、補足の説明などは不要です。" # <- この行は、ランキング結果のみ欲しい場合のみ有効にする
        f"\n {data}\n"
    )
